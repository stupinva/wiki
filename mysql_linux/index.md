...
Дополнительный плюс от использования огромных страниц заключается в том, что они не выгружаются в область подкачки.

Перед тем, как включить огромные страницы, нужно настроить пул страниц и группу пользователей, которым они будут доступны.

Посчитать размер пула и выставить группу можно с помощью следующего скрипта:

    #!/bin/sh
    
    innodb_buffer_pool_size=`mysql -BNe "SHOW GLOBAL VARIABLES WHERE variable_name='innodb_buffer_pool_size'" | awk '{ print $2; }'`
    innodb_buffer_pool_instances=`mysql -BNe "SHOW GLOBAL VARIABLES WHERE variable_name='innodb_buffer_pool_instances'" | awk '{ print $2; }'`
