...
К сожалению, на этом тестовом стенде агрегация сетевых интерфейсов работала загадочно. При деактивации сетевого интерфейса `virbr2` связь между виртуальными машинами оставалась, а вот при активации интерфейса `virbr1` с последующей деактивацией интерфейса `virbr2` связь пропадала. На виртуальной машине с CentOS, судя по содержимому файла `/proc/net/bonding/bond0`, не происходило согласование агрегации по протоколу LACP с партнёром по агрегации - виртуальной машиной с Debian.

Было подозрение, что сетевые мосты не транслируют трафик протокола LACP. В пользу этой версии говорило наблюдение трафика на сетевых интерфейсах vnet и virbr с помощью `tcpdump`:
