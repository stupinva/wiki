...
Для просмотра активных процессов можно воспользоваться таким запросом:
...
Просмотр списка включенных расширений
-------------------------------------
...
Досрочное завершение запросов
-----------------------------

Посмотреть запущенные в настоящее время запросы можно с поммощью следующего запроса:

    SELECT pid, state, usename, query FROM pg_stat_activity;


Для завершения запроса можно подставить значение из колонки pid в следующий запрос:
