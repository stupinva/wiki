...
В Debian для установки утилиты `ipmitool` достаточно установить одноимённый пакет:
...
### Восстановление доступа к /dev/ipmi0

На сервере модели Intel Corporation LWF2224IS814200 с материнской платой Intel Corporation S2600WFT с прошивкой модуля удалённого управления версии 2.22 столкнулся с проблемой, обратной описанной выше: доступ к веб-интерфейсу имеется, но загрузка модулей `ipmi_si` и `ipmi_devintf` не приводят к обнаружению модуля удалённого управления и появлению устройства `/dev/ipmi0`.

Проблему можно решить, имея доступ к веб-интерфейсу, следующим образом:

- Переходим в меню "Configuration"
- Открываем подменю "Security Settings"
- В выпадающем меню "KCS Mode" в блоке настроек "KCS Policy Control Mode" вместо значения "Deny All" выбираем значение "Restricted",
- Нажимаем кнопку "Save" внизу страницы.

Теперь можно попробовать выгрузить модули ядра IPMI и загрузить их снова:

    # rmmod ipmi_devintf ipmi_si ipmi_msghandler
    # modprobe ipmi_si ipmi_devintf

Если в каталоге `/proc/ipmi/` появился подкаталог с именем-цифрой N, то должно появиться и устройство с именем вида `/dev/ipmiN`. Можно попробовать воспользоваться утилитой ipmitool.

Для получения возможности управлять пользователями модуля удалённого управления из операционной системы нужно переключить режим безопасности в состояние "Allow All". После внесения необходимых изменений можно вернуть режим "Restricted".

### Высокая загрузка от kipmi0

На сервере модели Intel Corporation R2312GZ4GC4 с материнской платой Intel Corporation S2600GZ с прошивкой модуля удалённого управления версии 1.17 столкнулся с проблемой, описанной на странице [kipmi kernel helper thread kipmi0 is generating high CPU load](https://access.redhat.com/solutions/21322): утилита ipmitool не работает, при этом поток ядра с именем kipmiN (где N - цифра, совпадающая с номером устройства /dev/ipmiN) создаёт высокую нагрузку на процессор сервера (может полностью занять одно ядро процессора).
